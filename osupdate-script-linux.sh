...
# 2	Wrong usage
# 3     A command that does not upgrade packages failed (apt-get update, apt-get dist-upgrade -s, dnf check-update, yum check-update)
# 4	A command that upgrades packages failed (apt-get dist-upgrade, apt-get install, dnf upgrade, yum update)
...
dnf_perform_update() {
	...
	#if --security parameter is provided, perform only security patches
	#quoting of the variable containing options is ommited intentionally to enable word splitting	
	output="$(dnf $NONINTERACTIVE_OPTS $1 upgrade)"
	exit_code="$?"
	
	#if dnf returns 1 it means an error occurred but dnf was able to solve it
	[[ "$exit_code" -ne 0 && "$exit_code" -ne 1 ]] && { echo "$output" >&2; error "dnf $NONINTERACTIVE_OPTS $1 upgrade" "$exit_code" >&2; exit 4; }
}
...
# ======================================================================================
# =========================FUNCTIONS USING YUM (FOR CENTOS 7)===========================

# Function to display security/all updates based on parameter
yum_find_updates() {
	#set options based on positional parameter
	#-q is for omitting unnecessary information in the output
	#--security is for filtering security packages
	opt="-q"; [[ "$1" == "security" ]] && opt="-q --security"		

	#if check-update fails (exit code 100 means no updates have been found),
	#echo the error messages and exit the script
	#else parse the output and continue the script
	output="$(yum $opt check-update)"
	exit_code="$?"
	[[ "$exit_code" -ne 0 && "$exit_code" -ne 100 ]] && { echo "$output" >&2; error "yum $opt check-update" "$exit_code" >&2; exit 3; }
	printf '%s\n' "$output" | cut -d' ' -f1 | grep -v '^$' | sed 's/\.[^\.]*$//'
}

# Function to display security updates
yum_list_security_updates(){
	yum_find_updates "security"
}

# Function to display non-security updates
yum_list_other_updates() {
	#create temporary file
	tmpfile=$(mktemp) || { error "mktemp" "$?" >&2; exit 3; }
	
	#store the sorted list of all updates in the tmp file
	#sort is necessary for comm command to work
	yum_find_updates "all" | sort  > "$tmpfile"
	
	#use comm to display the lines that appear only in the tmpfile,
	#thus are not security updates
	yum_find_updates "security" | sort | comm -23 "$tmpfile" -
...
yum_perform_update() {
	...
	#if --security parameter is provided, perform only security patches
	#quoting of the variable containing options is ommited intentionally to enable word splitting	
	output="$(yum $NONINTERACTIVE_OPTS update $1)"
	exit_code="$?"
	[[ "$exit_code" -ne 0 ]] && { echo "$output"; error "yum $NONINTERACTIVE_OPTS update $1" "$exit_code" >&2; exit 4; }
}

# Function to perform security updates
yum_perform_security_updates(){
	yum_perform_update "--security"
}

# Function to perform all updates
yum_perform_all_updates(){
	yum_perform_update ""
}
...
if command -v apt-get &> /dev/null; then
	...
	echo 'None of the supported package managers (APT, DNF, YUM) found (by the "command" shell builtin)' 
	exit 1
fi	
